...
// Fast polynomial rolling hash
inline uint32_t compute_hash(std::string_view str) {
    uint32_t h = 0;
    for (char c : str) {
...
void execute_intentions(std::string base_intent, int repeats) {
    size_t N = base_intent.size();
    
    // We pre-allocate a buffer to hold [base_intent] [space] [max 11 chars for uint32]
    std::vector<char> buf(N + 1 + 12);
    
    // Write base_intent and space into the buffer once
    std::memcpy(buf.data(), base_intent.data(), N);
    buf[N] = ' ';

    for (int r = 0; r < repeats; ++r) {
        // Iteration 1 hashes the base intent string directly (e.g. "I am love")
        uint32_t h = compute_hash(std::string_view(buf.data(), N));

        // The remaining 88,888,887 iterations hash "base_intent" + " " + "previous_hash"
        for (int iter = 1; iter < 88888888; ++iter) {
            // Write the hash characters in-place starting at index N + 1 [1]
            auto [ptr, ec] = std::to_chars(buf.data() + N + 1, buf.data() + buf.size(), h);
            
            // Total length of the current combined string (e.g. "I am love 234234324")
            size_t current_len = ptr - buf.data();
            
            // Compute hash directly from the active slice of our buffer
            h = compute_hash(std::string_view(buf.data(), current_len));
        }

        // Store final loop state in atomic variable to enforce CPU utilization
